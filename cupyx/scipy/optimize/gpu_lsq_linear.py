"""GPU implementation of Linear least squares with bound constraints on independent variables."""
import numpy as np
...
from .gpu_common import all_in_bounds, in_bounds, compute_grad
from scipy.optimize._lsq.trf_linear import trf_linear
from .gpu_bvls import bvls
import time
...
def prepare_bounds(bounds, n):
    ...
    lb, ub = (cp.asarray(b, dtype=float) for b in bounds)
...
def gpu_lsq_linear(A, b, bounds=(-cp.inf, cp.inf), method='trf', tol=1e-10,
               lsq_solver=None, lsmr_tol=None, max_iter=None,
               verbose=0, *, lsmr_maxiter=None, return_cost=True,
               return_optimality=True, check_bounds=False):
    ...
    return_cost : True = return cost function, False = do not to save time
    return_optimality: True = return grad function, False = do not to save time
    check_bounds: True = check that lb < ub; False = do not to save time
    ...
    if lsq_solver is None:
        if isinstance(A, cp.ndarray):
            ...
            lsq_solver = 'lsmr'
    elif lsq_solver == 'exact' and not isinstance(A, cp.ndarray):
        raise ValueError("`exact` solver can't be used when `A` is "
                         "sparse or LinearOperator.")

    if len(A.shape) != 3:  # No ndim for LinearOperator.
        raise ValueError("`A` must have 3 dimensions.")
    ...
    if b.ndim != 2:
        raise ValueError("`b` must have at most 2 dimension.")
    ...
    if lb.shape != (n,) and ub.shape != (n,) and lb.shape != (z,n) and ub.shape != (z,n):
    ...
    if lsq_solver == 'exact':
        #unbd_lsq = cp.linalg.lstsq(A, b, rcond=-1)
        if m == n:
            #square matrices
            unbd_lsq = cp.linalg.solve(A,b)
        else:
            unbd_lsq = cp.einsum("ijk,ik->ij", cp.linalg.pinv(A), b)
    elif lsq_solver == 'lsmr':
        raise NotImplementedError('lsmr is not yet implemetned') 
    ...
    #Tile bounds if 1d arrays given
    if (lb.shape == (n,)):
        lb = cp.tile(lb, (z,1))
    if (ub.shape == (n,)):
        ub = cp.tile(ub, (z,1))
    ib = in_bounds(x_lsq, lb, ub)
    ...
    if ib.get().all():
        ...
        if (return_cost or return_optimality):
            r = cp.einsum("ijk,ik->ij", A, x_lsq) - b
            #r = A @ x_lsq - b
            cost = 0.5*cp.einsum("ij,ij->i", r, r)
            #cost = 0.5 * cp.dot(r, r)
            if (return_optimality):
                g = compute_grad(A, r)
                g_norm = norm(g, ord=cp.inf, axis=1)
            else:
                g_norm = None
        else:
            cost=None
        ...
        if verbose > 0:
            ...
            if (return_cost):
                print(f"Final cost {cost:.4e}")
            if (return_optimality):
                print(f"first-order optimality {g_norm:.2e}")
    ...
    if method == 'trf':
        raise NotImplementedError('trf_linear is not yet implemented')
        #res = trf_linear(A, b, x_lsq, lb, ub, tol, lsq_solver, lsmr_tol,
        #                 max_iter, verbose, lsmr_maxiter=lsmr_maxiter)
    elif method == 'bvls':
        res = bvls(A, b, x_lsq, lb, ub, ib, tol, max_iter, verbose,
               return_cost=return_cost, return_optimality=return_optimality)
