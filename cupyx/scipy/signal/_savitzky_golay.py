...
# https://github.com/scipy/scipy/blob/v1.10.1/scipy/_lib/_util.py#L166
...
# https://github.com/numpy/numpy/blob/v1.24.0/numpy/lib/polynomial.py#L704-L780
# inline instead of using `cupy.polyval` because the latter raises on
# non-1D arguments, which `np.polyval` accepts.
def _polyval(p, x):
    p = cupy.asarray(p)
    x = cupy.asanyarray(x)
    y = cupy.zeros_like(x)
