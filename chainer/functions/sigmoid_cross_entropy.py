...
class SigmoidCrossEntropy(function.Function):
    ...
    def forward_gpu(self, inputs):
        ...
        loss = -cuda.reduce(
            ['t', 'x'],
            'x[i] * (t[i] - (x[i] >= 0)) - log1p(exp(-fabs(x[i])))',
            'a + b', 0, 'sigmoid_crossent_fwd', x.dtype)(t, x)
        """
        loss = -cuda.cupy.sum(
            x * (t - (x >= 0)) - cuda.cupy.log1p(cuda.cupy.exp(-abs(x))),
            dtype=x.dtype)
        d = x * (t - (x >= 0)) - cuda.cupy.log1p(cuda.cupy.exp(-abs(x)))
        loss = -cuda.reduce(['x'],
            'x[i]',
            'a + b', 0, 'sigmoid_crossent_fwd', numpy.float32)(x)
        """
    ...
    def backward_gpu(self, inputs, grad_outputs):
        ...
        gx = cuda.elementwise(
            'T y, S t, raw T coeff', 'T gx',
            'gx = coeff[0] * (y - t)',
            'sigmoid_crossent_bwd')(self.y, t, coeff)
        return gx, None
