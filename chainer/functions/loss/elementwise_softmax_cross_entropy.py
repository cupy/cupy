...
class ElementwiseSoftmaxCrossEntropy(function.Function):

    """Softmax activation followed by a cross entropy loss."""

    ignore_label = -1

    def __init__(self, use_cudnn=True, cache_score=True,
                 class_weight=None):
    ...
    def forward_cpu(self, inputs):
        ...
        y = -(log_p * (t.ravel() != self.ignore_label)).reshape(t.shape)
        return y,
    ...
    def forward_gpu(self, inputs):
        ...
        ret = cuda.elementwise(
            ...
            ''',
            'elementwise_softmax_cross_entropy_fwd'
        ...
        if self.class_weight is None:
            gx = cuda.elementwise(
                ...
                'softmax_crossent_bwd')(
                    y, cupy.expand_dims(t, 1), gloss, x.shape[1], n_unit)
        else:
            gx = cuda.elementwise(
                ...
                ''',
                'softmax_crossent_bwd')(
                    y, self.class_weight, cupy.expand_dims(t, 1), gloss,
                    x.shape[1], n_unit)
        return gx, None


def elementwise_softmax_cross_entropy(
        x, t, use_cudnn=True, cache_score=True, class_weight=None):
    ...
    """
    return ElementwiseSoftmaxCrossEntropy(
        use_cudnn, cache_score, class_weight)(x, t)
