...
class BatchNormalizationFunction(function.Function):
    ...
    def __init__(self, mean, var, train, decay=0.9, eps=2e-5, use_cudnn=True):
        if train:
            ...
            self.fixed_var = var
        # If train is true, use batch statistics (training mode). Otherwise, if false, use the supplied
        # mean and variance.
        self.train = train
        # Note: cuDNN v5 requires that eps be greater than 1e-5. Otherwise, an error will occur.
        # See CUDNN_BN_MIN_EPSILON value in cudnn.h to verify minimum allowable value.
        self.eps = eps
    ...
    def forward(self, inputs):
        ...
        x, gamma, beta = inputs
        if (x.ndim == 4):
            self.mode = libcudnn.CUDNN_BATCHNORM_SPATIAL  # for convolutional layer
        else:
            self.mode = libcudnn.CUDNN_BATCHNORM_PER_ACTIVATION  # for linear layer
        ...
        if self.train:
            ...
            # Make sure the attribute arrays are the same type (numpy/cupy) as the inputs.
            self.fixed_mean = xp.array(self.fixed_mean)
            self.fixed_var = xp.array(self.fixed_var)

        if (x.ndim == 2) or (x.ndim == 4):
            # cuDNN only supports these tensor dimensions because they are the most commonly used. If there is
            # a need to support other dimensions with cuDNN, we could consider reshaping the input into a 2-dim array
            # with channels as second dim and m=<product off all dimensions except the 2nd dimension> as the first dimension.
            self.cudnn_dim_ok = True
        else:
            self.cudnn_dim_ok = False

        #use_cudnn_now = cuda.cudnn_enabled and self.use_cudnn and self.cudnn_dim_ok
        ...
        if xp is numpy:
            ...
            factor = 1 - self.decay # Factor used in the moving average computation runningMean
            if self.mean_cache is None:
                self.mean_cache = xp.zeros_like(gamma) # Output cache to speed up bacward pass (recommended to enable)
                self.var_cache = xp.zeros_like(gamma) # Output cache to speed up bacward pass (recommended to enable)
            ...
            if self.train:
                # Note: cuDNN computes the mini-batch mean and variance internally. We can simply (optionally) pass
                # it the running-average mean and variance arrays.
                libcudnn.batchNormalizationForwardTraining(handle, self.mode, one.data, zero.data,
                                                       x_desc.value, x.data.ptr, x_desc.value, y.data.ptr,
                                                       derivedBnDesc.value, gamma.data.ptr, beta.data.ptr,
                                                       factor, self.running_mean.data.ptr, self.running_var.data.ptr,
                                                       self.eps, self.mean_cache.data.ptr, self.var_cache.data.ptr)
                cudnn_updated_running_stats = True
            else:
                libcudnn.batchNormalizationForwardInference(handle, self.mode, one.data, zero.data,
                                                       x_desc.value, x.data.ptr, x_desc.value, y.data.ptr,
                                                       derivedBnDesc.value, gamma.data.ptr, beta.data.ptr,
        ...
        if self.train and (not cudnn_updated_running_stats):
            # Note: If in training mode, the cuDNN forward training function will do this for us, so
    ...
    def backward(self, inputs, grad_outputs):
        ...
        if xp is numpy:
            ...
            if self.train:
                ...
                # Note: Under normal conditions, this will never be executed because fixed-mean-variance mode is normally
                # only used at test/evaluation time.
                gx = (gamma / self.std)[expander] * gy
        elif cuda.cudnn_enabled and self.use_cudnn and self.cudnn_dim_ok and self.train:
            # Note: cuDNN batch normalization backward only works in "training mode." That is, it does not support
            # computing gradients in fixed-mean-variance mode, because there is normally no reason to call backward()
            ...
            libcudnn.batchNormalizationBackward(handle, self.mode, one.data, zero.data,
            ...
            if self.train:
                ...
                # Note: Under normal conditions, this will never be executed because fixed-mean-variance mode is normally
                # only used at test/evaluation time.
                # If cuDNN was called on the forward call, self.x_hat will be None.
                if self.x_hat is None:
                    self.std = xp.sqrt(self.fixed_var, dtype=self.fixed_var.dtype)
...
def batch_normalization(x, gamma, beta, running_mean=None, running_var=None, decay=0.9, eps=2e-5):
    ...
    Note: If this function is called, it will not be possible to access the updated running mean and variance
    statistics, because they are members of the function object, which cannot be accessed by the caller.
    If it is desired to access the updated running statistics, it is necessary to get a new instance of the
    function object, call the object, and then access the running_mean and/or running_var attributes. See the
    ...
    Args:
        ...
        running_mean (array): The running average of the mean. This is a running average of the mean over several
            mini-batches using the decay parameter. If None, the running average is not computed. If this is None,
            then runnng_var must also be None.
        running_var (array): The running average of the variance. This is a running average of the variance
            over several mini-batches using the decay parameter. If None, the running average is not computed. If
    ...
    """
    return BatchNormalizationFunction(running_mean, running_var, True, decay, eps)(x, gamma, beta)
...
def fixed_batch_normalization(x, gamma, beta, fixed_mean, fixed_var, eps=2e-5):
    ...
    statistics are given by the caller as fixed parameters. This is used on testing
    ...
    """
    return BatchNormalizationFunction(fixed_mean, fixed_var, False, 0.0, eps)(x, gamma, beta)
