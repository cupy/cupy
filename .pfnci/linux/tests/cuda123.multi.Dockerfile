...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    apt-get -qqy --allow-change-held-packages \
            --allow-downgrades install 'libnccl2=2.19.*+cuda12.3' 'libnccl-dev=2.19.*+cuda12.3' 'libcutensor2-cuda-12=2.3.*'
...
RUN pip install -U 'numpy==2.1.*' 'scipy==1.16.*' 'optuna==3.*' 'mpi4py==3.*' 'cython==3.0.*' 'fastrlock>=0.5'
