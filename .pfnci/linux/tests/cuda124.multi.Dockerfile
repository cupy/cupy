...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    apt-get -qqy --allow-change-held-packages \
            --allow-downgrades install 'libnccl2=2.20.*+cuda12.4' 'libnccl-dev=2.20.*+cuda12.4' 'libcutensor2-cuda-12=2.3.*'
...
RUN pip install -U 'numpy==2.2.*' 'scipy==1.16.*' 'optuna==3.*' 'mpi4py==3.*' 'cython==3.0.*' 'fastrlock>=0.5'
