...
class GaussianMixture(object):
    ...
    def _compute_precision_chol(self, covariances):
    ...
    def _initialize(self, X, resp, n_samples):
        ...
        self.precisions_chol_ = \
            self._compute_precision_chol(covariances)

    def _initialize_parameters(self, X, n_samples, random_state):
        # add kmeans pattern
        resp = random_state.rand(n_samples, self.n_components)
        resp /= self.xp.sum(resp, axis=1)[:, self.xp.newaxis]
        self._initialize(X, resp, n_samples)

    def _estimate_log_prob(self, X, means, precisions_chol):
    ...
    def _e_step(self, X):
        weighted_log_prob = self._estimate_log_prob(
            X, self.means_, self.precisions_chol_) + self.xp.log(self.weights_)
    ...
    def _m_step(self, X, log_resp):
        ...
        self.precisions_chol_ = self._compute_precision_chol(self.covariances_)
    ...
    def fit(self, X, y=None):
        ...
        converged = False
        n_samples = X.shape[0]
        self._initialize_parameters(X, n_samples, random_state)
        ...
        self.precisions_ = self.precisions_chol_ ** 2
        self.n_iter_ = best_n_iter

        return self

    def predict(self, X):
        result = self._estimate_log_prob(X, self.means_, self.precisions_chol_)
