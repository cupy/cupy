...
class GoogLeNetBNFp16(GoogLeNetBN):
    ...
    def __init__(self):
        ...
        chainer.Chain.__init__(
            self,
            conv1=L.Convolution2D(3, 64, 7, stride=2, pad=3, initialW=W,
                                  nobias=True),
            norm1=L.BatchNormalization(64, dtype=dtype),
            conv2=L.Convolution2D(64, 192, 3, pad=1, initialW=W, nobias=True),
            ...
            inc5b=L.InceptionBN(1024, 352, 192, 320, 192, 224, 'max', 128,
                                conv_init=W, dtype=dtype),
            out=L.Linear(1024, 1000, initialW=W, bias=bias),

            conva=L.Convolution2D(576, 128, 1, initialW=W, nobias=True),
            norma=L.BatchNormalization(128, dtype=dtype),
            lina=L.Linear(2048, 1024, initialW=W, nobias=True),
            norma2=L.BatchNormalization(1024, dtype=dtype),
            outa=L.Linear(1024, 1000, initialW=W, bias=bias),

            convb=L.Convolution2D(576, 128, 1, initialW=W, nobias=True),
            normb=L.BatchNormalization(128, dtype=dtype),
            linb=L.Linear(2048, 1024, initialW=W, nobias=True),
