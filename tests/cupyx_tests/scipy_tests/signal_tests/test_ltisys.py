...
class Test_abcd_normalize:
    ...
    def test_A_nosquare_fails(self):
        assert_raises(
            ValueError, abcd_normalize, [1, -1], self.B, self.C, self.D
        )

    def test_AB_mismatch_fails(self):
        assert_raises(
            ValueError, abcd_normalize, self.A, [-1, 5], self.C, self.D
        )
    ...
    def test_CD_mismatch_fails(self):
        assert_raises(
            ValueError, abcd_normalize, self.A, self.B, self.C, [2.5, 0]
        )
...
class TestLsim:
    ...
    def test_nonequal_timesteps(self):
        ...
        system = ([1.0], [1.0, 0.0])
        with assert_raises(
            ValueError, match="Time steps are not equally spaced."
        ):
...
class TestPlacePoles:
    ...
    @pytest.mark.xfail(
        sys.platform.startswith("win32") or platform.processor() == "aarch64",
    ...
    def test_complex(self, xp, scp):
        ...
        # editions p 184/185
        A = xp.array(
            [[0, 7, 0, 0], [0, 0, 0, 7 / 3.0], [0, 0, 0, 0], [0, 0, 0, 0]]
        )
        B = xp.array([[0, 0], [0, 0], [1, 0], [0, 1]])
    ...
    def test_complex_4(self, xp, scp):
        ...
        P = xp.array(
            [-10, -20, -30, 40, 50, 60, 70, -20 - 5j, -20 + 5j, 5 + 3j, 5 - 3j]
        )
    ...
    def test_errors(self):
        ...
        with assert_raises(ValueError):
            signal.place_poles(
                A, B, cupy.array([-2.1, -2.2, -2.3, -2.4]), method="foo"
            )
