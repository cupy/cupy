...
class TestVecdot(unittest.TestCase):
    ...
    def test_vecdot(self, xp, dtype):
        ...
        try:
            ...
            if xp is numpy and not hasattr(numpy, 'vecdot'):
                ...
                # vecdot broadcasting is a bit complex to mock perfectly
                # with sum(a*b) but for these test cases it should work
                # if shapes are same after moving axis
    ...
    def test_vecdot_out(self, xp, dtype):
        ...
        # Compute expected shape of output
        # _GUFunc signature (n),(n)->() means output shape is the
        # broadcast of outer dimensions.
        a1_m = xp.moveaxis(a1, axis, -1)
        a2_m = xp.moveaxis(a2, axis, -1)
        out_shape = xp.broadcast(a1_m[..., 0], a2_m[..., 0]).shape
        out = xp.empty(out_shape, dtype=dtype)

        try:
            res = xp.vecdot(a1, a2, axis=axis, out=out)
            assert res is out
            return res
        except (ValueError, AttributeError):
            if xp is numpy and not hasattr(numpy, 'vecdot'):
                ...
                a1_m = xp.moveaxis(a1_conj, axis, -1)
                a2_m = xp.moveaxis(a2, axis, -1)
                res = (a1_m * a2_m).sum(axis=-1, out=out)
                return res
            raise
