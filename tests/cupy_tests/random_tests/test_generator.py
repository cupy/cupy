...
class RandomGeneratorTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rs = _random_state.RandomState(seed=self.__seed)
...
class TestRandomState(unittest.TestCase):
    ...
    def setUp(self):
        self.rs = _random_state.RandomState(seed=testing.generate_seed())
...
class TestResetStates(unittest.TestCase):
    ...
    def test_reset_states(self):
        _random_state._random_states = 'dummy'
        _random_state.reset_states()
        assert {} == _random_state._random_states
...
class TestGetRandomState(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rs_tmp = _random_state._random_states

    def tearDown(self, *args):
        _random_state._random_states = self.rs_tmp

    def test_get_random_state_initialize(self):
        _random_state._random_states = {}
        rs = _random_state.get_random_state()
        assert _random_state._random_states[self.device_id] == rs

    def test_get_random_state_memoized(self):
        _random_state._random_states = {self.device_id: 'expected',
                                        self.device_id + 1: 'dummy'}
        rs = _random_state.get_random_state()
        assert 'expected' == _random_state._random_states[self.device_id]
        assert 'dummy' == _random_state._random_states[self.device_id + 1]
        assert 'expected' == rs
...
class TestSetRandomState(unittest.TestCase):
    ...
    def test_set_random_state(self):
        rs = _random_state.RandomState()
        _random_state.set_random_state(rs)
        assert _random_state.get_random_state() is rs

    def test_set_random_state_call_multiple_times(self):
        _random_state.set_random_state(_random_state.RandomState())
        rs = _random_state.RandomState()
        _random_state.set_random_state(rs)
        assert _random_state.get_random_state() is rs
...
class TestGetRandomState2(unittest.TestCase):
    ...
    def setUp(self):
        self.rs_dict = _random_state._random_states
        _random_state._random_states = {}
        self.cupy_seed = os.getenv('CUPY_SEED')

    def tearDown(self, *args):
        _random_state._random_states = self.rs_dict
    ...
    def test_get_random_state_with_cupy(self):
        rvs0 = self._get_rvs(_random_state.RandomState(6))
    ...
    def _get_rvs_reset(self):
        _random_state.reset_states()
        return self._get_rvs(_random_state.get_random_state())
...
class TestCheckAndGetDtype(unittest.TestCase):
    ...
    def test_float32_64_type(self, dtype):
        assert _random_state._check_and_get_dtype(dtype) == numpy.dtype(dtype)

    def test_float16(self):
        with self.assertRaises(TypeError):
            _random_state._check_and_get_dtype(numpy.float16)

    @testing.for_int_dtypes()
    def test_int_type(self, dtype):
        with self.assertRaises(TypeError):
            _random_state._check_and_get_dtype(dtype)
