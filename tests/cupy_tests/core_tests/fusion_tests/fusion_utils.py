...
def check_fusion(
    ...
    def deco(func):
        def wrapper(self, **generate_inputs_kwargs):
            ...
            result_np = run(impl_np, args_np, kwargs_np)
            result_cp = run(impl_cp, args_cp, kwargs_cp)
            result_fuse_np = run(impl_fuse_np, args_fuse_np, kwargs_fuse_np)
            result_fuse_cp = run(impl_fuse_cp, args_fuse_cp, kwargs_fuse_cp)
