...
class TestIndexing(unittest.TestCase):
    ...
    def generate_inputs(self, xp, dtype1, dtype2):
        x = testing.shaped_random((3, 4), xp, dtype1, scale=10, seed=0)
        # TODO(asi1024): Support array indexing.
        indices = testing.shaped_random((3, 4), xp, dtype2, scale=10, seed=1)
        return (x, indices), {}

    @testing.for_all_dtypes(name='dtype1')
    @testing.for_int_dtypes(name='dtype2')
    @fusion_utils.check_fusion()
    def test_getitem_scalar(self, xp, dtype1, dtype2):
        return lambda x, y: x[0]

    @testing.for_all_dtypes(name='dtype1')
    @testing.for_int_dtypes(name='dtype2')
    @fusion_utils.check_fusion()
    def test_getitem_tuple(self, xp, dtype1, dtype2):
        return lambda x, y: x[0, 1]
