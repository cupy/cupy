...
_test_name_expr_cache = r'''
namespace mylib { enum class Tag { A, B }; }

template<typename Tv, typename Tp>
__global__ void multi_param() { }

template<typename T>
__global__ void ptr_param(T* p) { }

template<typename T, int N>
__global__ void non_type_param() { }

template<mylib::Tag t>
__global__ void enum_param() { }
'''
...
@contextlib.contextmanager
def _counting_cache_dir():
    target1 = 'cupy.cuda.compiler._kernel_cache_backend'
    target2 = 'cupy.cuda.compiler._empty_file_preprocess_cache'
    with tempfile.TemporaryDirectory() as path:
        backend = _CountingCacheBackend(path)
        with mock.patch(target1, backend):
            with mock.patch(target2, {}):
                yield backend
...
class TestCachePayload:
    ...
    def test_round_trip(self):
        names = ['_Z6kernelIffEvv', '_Z6kernelIfdEvv']
        payload = compiler._encode_cache_payload(names, b'\x7fELFcubin')
        assert compiler._decode_cache_payload(payload, 2) == (
            names, b'\x7fELFcubin')

    def test_no_names(self):
        payload = compiler._encode_cache_payload([], b'cubin')
        assert compiler._decode_cache_payload(payload, 0) == ([], b'cubin')

    def test_count_mismatch_is_rejected(self):
        payload = compiler._encode_cache_payload(['_Z1kv'], b'cubin')
        assert compiler._decode_cache_payload(payload, 2) is None

    # entries written before this format existed, and cubins/PTX stored
    # for modules without name expressions, must not be misread
    @pytest.mark.parametrize('payload', [
        b'', b'\x7fELF and then some cubin', b'//\n// Generated by NVIDIA\n'])
    def test_foreign_payload_is_rejected(self, payload):
        assert compiler._decode_cache_payload(payload, 1) is None

    def test_truncated_payload_is_rejected(self):
        payload = compiler._encode_cache_payload(['_Z1kv'], b'cubin')
        for size in range(len(compiler._cache_payload_magic), len(payload)):
            if compiler._decode_cache_payload(payload[:size], 1) is not None:
                # truncating inside the cubin is indistinguishable from a
                # short cubin, so only the header needs to be rejected
                assert size >= len(payload) - len(b'cubin')


@pytest.mark.skipif(
    cupy.cuda.runtime.is_hip,
    reason="HIP does not cache modules built with name_expressions")
class TestRawNameExpressionCache:

    # Spellings that the demangled-name matcher used to reconcile against.
    # The second of each pair is the one a user is likely to write; the
    # first is the form __cu_demangle happens to emit.
    @pytest.mark.thread_unsafe(reason="mutates global cache directory")
    @pytest.mark.parametrize('name_expression', [
        'multi_param<float, double>',
        'multi_param<float,double>',
        'ptr_param<float *>',
        'ptr_param<float*>',
        'non_type_param<float, (int)3>',
        'non_type_param<float, 3>',
        # size_t and Tag::B are erased by mangling: the cubin records
        # only "unsigned long" and "(mylib::Tag)1", so no amount of
        # string matching can recover what the user typed.
        'ptr_param<unsigned long>',
        'ptr_param<size_t>',
        'enum_param<(mylib::Tag)1>',
        'enum_param<mylib::Tag::B>',
    ])
    def test_cubin_cache_is_reused(self, name_expression):
        with _counting_cache_dir() as backend:
            ...
            # drop the in-process memo so the second build has to
            # consult the on-disk cache
            _util.clear_memo()
