...
@pytest.mark.thread_unsafe(reason="uses temporary cache dir")
class TestNameExpressionsCaching(unittest.TestCase):
    """Test that name_expressions with caching works correctly.
    
    This test verifies that the CUDA 11.6+ function enumeration APIs
    (cuModuleEnumerateFunctions, cuFuncGetName) properly extract kernel
    names from cached CUBIN files, eliminating the need to recompile.
    """

    def test_cache_hit_with_name_expressions(self):
        """Test that second compilation with name_expressions uses cache."""
        test_template = r'''
        template<typename T>
        __global__ void my_kernel(T *x, int N) {
            int tid = blockDim.x * blockIdx.x + threadIdx.x;
            if (tid < N) {
                x[tid] = x[tid] * x[tid];
            }
        ...
        name_expressions = ['my_kernel<int>', 'my_kernel<float>']
        
        with use_temporary_cache_dir():
            # First compilation - should compile and cache
            mod1 = cupy.RawModule(code=test_template,
                                  name_expressions=name_expressions)
            
            # Get a kernel and verify it works
            ker1 = mod1.get_function('my_kernel<int>')
            in_arr = cupy.array([1, 2, 3, 4], dtype=cupy.int32)
            ker1((1,), (4,), (in_arr, 4))
            expected = cupy.array([1, 4, 9, 16], dtype=cupy.int32)
            assert cupy.allclose(in_arr, expected)
            
            # Verify mapping was set during compilation
            assert mod1.module.mapping is not None
            assert 'my_kernel<int>' in mod1.module.mapping
            assert 'my_kernel<float>' in mod1.module.mapping
            
            # Second compilation - should use cache
            mod2 = cupy.RawModule(code=test_template,
                                  name_expressions=name_expressions)
            
            # Get a kernel and verify it works
            ker2 = mod2.get_function('my_kernel<float>')
            in_arr2 = cupy.array([1.0, 2.0, 3.0, 4.0], dtype=cupy.float32)
            ker2((1,), (4,), (in_arr2, 4))
            expected2 = cupy.array([1.0, 4.0, 9.0, 16.0], dtype=cupy.float32)
            assert cupy.allclose(in_arr2, expected2)
            
            # Verify mapping was built from cached CUBIN via enumeration
            assert mod2.module.mapping is not None
            assert 'my_kernel<int>' in mod2.module.mapping
            assert 'my_kernel<float>' in mod2.module.mapping
            
            # The mangled names should be non-empty strings
            for name_expr in name_expressions:
                mangled = mod2.module.mapping[name_expr]
                assert isinstance(mangled, str)
                assert len(mangled) > 0

    def test_demangling_and_matching(self):
        """Test that C++ demangling properly matches name expressions."""
        test_template = r'''
        template<typename T>
        __global__ void square(T *x, int N) {
            int tid = blockDim.x * blockIdx.x + threadIdx.x;
            if (tid < N) {
                x[tid] = x[tid] * x[tid];
            }
        ...
        # Test with different whitespace in name expression
        name_expressions = ['square<int>', 'square< float >']
        
        with use_temporary_cache_dir():
            mod = cupy.RawModule(code=test_template,
                                 name_expressions=name_expressions)
            
            # Both should match despite different whitespace
            assert mod.module.mapping is not None
            assert 'square<int>' in mod.module.mapping
            assert 'square< float >' in mod.module.mapping
            
            # Test that both kernels work
            ker_int = mod.get_function('square<int>')
            ker_float = mod.get_function('square< float >')
            
            in_int = cupy.array([2, 3], dtype=cupy.int32)
            ker_int((1,), (2,), (in_int, 2))
            assert cupy.allclose(in_int, cupy.array([4, 9], dtype=cupy.int32))
            
            in_float = cupy.array([2.0, 3.0], dtype=cupy.float32)
            ker_float((1,), (2,), (in_float, 2))
            assert cupy.allclose(in_float, cupy.array([4.0, 9.0], dtype=cupy.float32))
