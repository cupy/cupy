...
a different input argument from that of :class:`~cupy.cuda.MemoryPool`: the default (``None``) is to use the device's default
memory pool. It also allows using the device's *current* memory pool (which could be different from the default one), or passing
in an ``int`` representing a pool handle ``cudaMemPool_t`` to use the memory pool created by other libraries. Please refer to
