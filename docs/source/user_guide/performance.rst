...
The ``cuda_compute`` backend uses `cuda.compute <https://nvidia.github.io/cccl/python/compute.html>`_ (from the optional `cuda-cccl <https://pypi.org/project/cuda-cccl/>`_ package) and currently accelerates inclusive scans (:func:`~cupy.cumsum` and :func:`~cupy.cumprod`); setting ``CUPY_ACCELERATORS=cuda_compute,cub`` tries cuda.compute for cumsum/prod while keeping CUB for everything else.
