...
The ``cuda_compute`` backend uses `cuda.compute <https://nvidia.github.io/cccl/python/compute.html>`_ (from the `cuda-cccl <https://pypi.org/project/cuda-cccl/>`_ package) and currently accelerates reductions (ex: :func:`~cupy.sum`, :func:`~cupy.max` and :func:`~cupy.argmax`), inclusive scans (:func:`~cupy.cumsum` and :func:`~cupy.cumprod`) and :func:`~cupy.bincount`. Setting ``CUPY_ACCELERATORS=cuda_compute,cub`` tries ``cuda.compute`` for these while keeping CUB for everything else.
