...
**Batch conversion functions** convert the mini batch into arrays to feed to the neural nets. They are also responsible to send each array to an appropriate device. Chainer currently provides :func:`concat_example` as the only example of batch conversion functions.
