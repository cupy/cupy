...
.. envvar:: CUPY_ACCELERATORS
  ...
  A comma-separated string of backend names (``cub``, ``cutensor``, or ``cutensornet``) which indicates the acceleration backends used in CuPy operations and its priority.
  All accelerators are disabled by default on HIP, and only CUB is enabled on CUDA.
