...
When major update of CuPy, numbers generated by `cupy.random` of new version
may not be same as that of previous version even if the same seed and
distribution is used. However this does not mean distribution from which
numbers is generated is wrong.
