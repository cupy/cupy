...
/* This function should be changed as soon as FreeBSD's msun gets a cpow
 * function */
template <>
__device__ inline complex<double> pow(const complex<double>& z,
...
__device__ inline complex<typename _select_greater_type<T, U>::type> pow(
  ...
  return thrust::exp(thrust::log(complex<PromotedType>(z)) *
                     complex<PromotedType>(exponent));
}
