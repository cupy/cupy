            ...
            if not (out.flags.c_contiguous and self._c_contiguous or
                ...
                with self.device:
                    if out.flags.c_contiguous:
                        a_gpu = _creation.internal_ascontiguousarray(self)
                    elif out.flags.f_contiguous:
                        a_gpu = _creation.internal_asfortranarray(self)
                    else:
                        raise RuntimeError(
...
cpdef ndarray matmul(ndarray a, ndarray b, ndarray out=None):
    ...
    a = _creation.ascontiguousarray(a, dtype)
    b = _creation.ascontiguousarray(b, dtype)
