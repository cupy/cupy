...
cpdef ndarray matmul(ndarray a, ndarray b):
    """
    .. note::
        This function is a clone of numpy.matmul and the @ operator since
        python 3.5.
        Differences: Currently the output must be float32 (float64, comlplex64
        and complex128 follow later). This means, that
        numpy.result_type(a.dtype, b.dtype) have to be numpy.float32.

        The out array is currently not supported.

        The broadcasting rules are implemented.


    Numpy dokumentation:

    cupy.matmul(a, b, out=None)
        ...
        out : ndarray, optional
            ...
            returned if it was not used. In particular, it must have the right
            type, must be C-contiguous, and its dtype must be the dtype that
            would be returned for `dot(a,b)`. This is a performance feature.
