...
cdef class _Chunk:
    ...
    Notes:
        Mutating chunks may only be safe if the arena mutex is held.
        Normally it is the arena mutating them (e.g. triggering a merge).

        Chunks own the _Arena, which ensures that the arena is not cleaned up
        while we still have existing chunks for it.
        OTOH, it also means that we must set ``arena = None`` when freeing
        the chunk from the arena (i.e. freeing it).
    """
    ...
    cdef:
        ...
        readonly _Arena arena  # also ensures only an empty arena is deleted.
        public _Chunk prev
        public _Chunk next
