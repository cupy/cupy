...
def is_after_baseline(
    ...
    """Returns True if the given version is newer than baseline.
...
def skip_if_after_baseline(
    ...
    def wrapper(func: Callable) -> Callable:
        if numpy is not None and is_after_baseline(numpy=numpy):
            func = _skipif(installed("numpy>=" + numpy), reason=reason)(func)
        if scipy is not None and is_after_baseline(scipy=scipy):
            func = _skipif(installed("scipy>=" + scipy), reason=reason)(func)
        return func
